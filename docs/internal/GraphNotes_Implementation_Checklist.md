...
- [x] Resolve app data path (use `tauri::Manager::path().app_data_dir()`)
- [x] Initialize DB connection on app startup: `Surreal::new::<SurrealKv>(path)`, use_ns/use_db
- [x] Define schema via SurrealQL `DEFINE TABLE` / `DEFINE FIELD` / `DEFINE INDEX` statements for `note`, `tag`, `dangling_node`, `links_to`, `tagged_with`
- [x] Store DB handle in `AppState`
